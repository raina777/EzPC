...
void NeuralNetwork::forward()
{
	...
	// if (PRIMARY)
	// 	funcReconstruct2PC(*(layers[LLL]->getActivation()), (*(layers[LLL]->getActivation())).size(), "LLL: activations");
}
...
void NeuralNetwork::computeDelta()
{
	...
	for (size_t i = 0; i < rows; ++i)
		for (size_t j = 0; j < columns; ++j)
			rowSum[i*columns] += (*(layers[LLL]->getActivation()))[i * columns + j];
	...
	if (STANDALONE)
	{
		for (size_t i = 0; i < rows; ++i)
			for (size_t j = 0; j < columns; ++j)
			{
				index = i * columns + j;
				if (rowSum[index] != 0)
					quotient[index] = divideMyTypeSA((*(layers[LLL]->getActivation()))[index], rowSum[index]);
			}
	}
	else
	{
		funcDivisionMPC(*(layers[LLL]->getActivation()), rowSum, quotient, size);
	}
...
//WITHOUT DIVISION BEGINS HERE
	// for (size_t i = 0; i < rows; ++i)
	// 	for (size_t j = 0; j < columns; ++j)
	// 		quotient[i * columns + j] += (*(layers[LLL]->getActivation()))[i * columns + j];
...
//WITHOUT DIVISION ENDS HERE
	...
	for (size_t i = 0; i < rows; ++i)
		...
		{
			index = i * columns + j;
			(*(layers[LLL]->getDelta()))[index] = quotient[index] - outputData[index];
		}

	for (size_t i = LLL; i > 0; --i)
...
void NeuralNetwork::predict(vector<myType> &maxIndex)
{
	...
	layers[LLL]->findMax(*(layers[LLL]->getActivation()), max, maxIndex, rows, columns);
}
...
void NeuralNetwork::getAccuracy(const vector<myType> &maxIndex, vector<size_t> &counter)
{
	...
	layers[LLL]->findMax(outputData, max, groundTruth, rows, columns);
