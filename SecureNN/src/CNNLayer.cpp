...
void CNNLayer::forward(vector<myType>& inputActivation)
{
	...
	if (MPC)
	{
		funcMatMulMPC(convShaped, reshapedWeights, convOutput, 
					(p_range*q_range*B), (fw*fh*C), D, 0, 0, 1 /*coming_from_conv*/);
	}
#endif	
	//Change Mayank
	//Convolution optimization
#ifdef CONV_OPTI
#ifdef USE_CONV_2DCSF
	int n, ci, co, zPadH, zPadW, strideH, strideW, sf;
	n = B; // No of input images
	ci = C; // Input channels
	co = D; // Output channels
	zPadH = 0; // Zero padding along height
	zPadW = 0; // Zero padding along width
	strideH = 1; // Stride along height
	strideW = 1; // Stride along width
	sf = 1; // Scaling factor

	//vector<myType> inputArr(n*ih*iw*ci);
	//vector<myType> filterArr(fh*fw*ci*co);
	int reshapedFilterRows = co;
	int reshapedFilterCols = fh*fw*ci;
	int reshapedInputRows = fh*fw*ci;
	int reshapedInputH = (((ih+(2*zPadH)-fh)/strideH)+1);
	int reshapedInputW = (((iw+(2*zPadW)-fw)/strideW)+1);
	int reshapedInputCols = n*reshapedInputH*reshapedInputW;
	vector<myType> convOutput(n*reshapedInputH*reshapedInputW*co);
	
	vector<vector<vector<vector<myType>>>> rInput(B, vector<vector<vector<myType>>>(ih, vector<vector<myType>>(iw, vector<myType>(ci))));
	vector<vector<vector<vector<myType>>>> rFilter(fh, vector<vector<vector<myType>>>(fw, vector<vector<myType>>(ci, vector<myType>(co))));
	vector<vector<vector<vector<myType>>>> rOutput(n, vector<vector<vector<myType>>>(reshapedInputH, vector<vector<myType>>(reshapedInputW, vector<myType>(co))));

	deflatten_4D_vector(inputActivation, rInput, B, ih, iw, ci);
	deflatten_4D_vector(weights, rFilter, fh, fw, ci, co);

	funcConv2DCSF(B, ih, iw, ci /*input channels*/, fh, fw, co /*output channels*/, zPadH /*z pad h*/, zPadW /*z pad w*/, strideH /*stride h*/, strideW /*stride w*/, rInput, rFilter, rOutput, sf /*scaling factor*/);

	flatten_4D_vector(rOutput, convOutput, n, reshapedInputH, reshapedInputW, co);
	
#endif
#endif

#ifdef CONV_OPTI
#ifndef USE_CONV_2DCSF
	vector<myType> convOutput(p_range*q_range*B*D, 0);
	testConvolutionOptimized(iw, ih, fw, fh, B, C, D, 1, convOutput, weights, inputActivation);		
#endif
#endif
