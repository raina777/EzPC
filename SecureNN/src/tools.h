...
template<typename T1, typename T2>
void addModuloOddArr(T1* a, T2* b, myType* c, size_t size);

template<typename T1, typename T2>
void subtractModuloOddArr(T1* a, T2* b, myType* c, size_t size);
...
{	
	...
	if (r_type == "COMMON")
	{
		...
		{		
			...
			{
#ifdef PRECOMPUTEAES
				aes_common->fillWithRandomBits64((uint64_t*)vec.data(), size);
	...
	if (r_type == "INDEP")
	{
		...
		{		
			...
			{
#ifdef PRECOMPUTEAES
				aes_indep->fillWithRandomBits64((uint64_t*)vec.data(), size);
				for (size_t i = 0; i < size; ++i)
					vec[i] *= sign;			
#else
				for (size_t i = 0; i < size; ++i)
...
#ifdef PRECOMPUTEAES
		aes_a_1->fillWithRandomBits64((uint64_t *)vec.data(), size);
#else
		for (size_t i = 0; i < size; ++i)
...
#ifdef PRECOMPUTEAES
		aes_b_1->fillWithRandomBits64((uint64_t *)vec.data(), size);
#else
		for (size_t i = 0; i < size; ++i)
...
#ifdef PRECOMPUTEAES
		aes_c_1->fillWithRandomBits64((uint64_t *)vec.data(), size);
#else
		for (size_t i = 0; i < size; ++i)
...
#ifdef PRECOMPUTEAES
		aes_a_2->fillWithRandomBits64((uint64_t *)vec.data(), size);
#else
		for (size_t i = 0; i < size; ++i)
...
#ifdef PRECOMPUTEAES
		aes_b_2->fillWithRandomBits64((uint64_t *)vec.data(), size);
#else
		for (size_t i = 0; i < size; ++i)
...
#ifdef PRECOMPUTEAES
		aes_conv_opti_a_1->fillWithRandomBits64((uint64_t *)vec.data(), size);
#else
		for (size_t i = 0; i < size; ++i)
...
#ifdef PRECOMPUTEAES
		aes_conv_opti_b_1->fillWithRandomBits64((uint64_t *)vec.data(), size);
#else
		for (size_t i = 0; i < size; ++i)
...
#ifdef PRECOMPUTEAES
		aes_conv_opti_c_1->fillWithRandomBits64((uint64_t *)vec.data(), size);
#else
		for (size_t i = 0; i < size; ++i)
...
#ifdef PRECOMPUTEAES
		aes_conv_opti_a_2->fillWithRandomBits64((uint64_t *)vec.data(), size);
#else
		for (size_t i = 0; i < size; ++i)
...
#ifdef PRECOMPUTEAES
		aes_conv_opti_b_2->fillWithRandomBits64((uint64_t *)vec.data(), size);
#else
		for (size_t i = 0; i < size; ++i)
...
template<typename T>
void fillVector(T val, vector<T> c, size_t size)
{
	for (size_t i = 0; i < size; i++)
	{
		c[i] = val;
...
void subtractModuloOddArr(T1* a, T2* b, myType* c, size_t size)
{
	...
	addModuloOddArr<T1, myType>(a, temp, c, size);
}
...
void receive_2_4D_vector(vector< vector< vector< vector<myType> > > >& recv1,
						vector< vector< vector< vector<myType> > > >& recv2,
						int d11, int d12, int d13, int d14,
						int d21, int d22, int d23, int d24);
...
template<typename T>
T** mallocDyn2DArr(int32_t s0, int32_t s1)
{
	T** arr = new T*[s0];
	for(int i=0;i<s0;i++)
		arr[i] = new T[s1];
...
template<typename T>
T**** mallocDyn4DArr(int32_t s0, int32_t s1, int32_t s2, int32_t s3)
{
	T**** arr = new T***[s0];
	for(int i=0;i<s0;i++){
		arr[i] = new T**[s1];
