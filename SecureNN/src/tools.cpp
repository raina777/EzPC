...
void print_usage (const char * bin) 
{
    ...
    cout << endl;
    cout << "Report bugs to t-niskum@microsoft.com" << endl;
    exit(-1);
}
				 	...
				 	size_t transpose_a, size_t transpose_b)
{
	...
	//Fill row major and col major arrays.
	//print_string("Mutlithreaded matmul");
	int offset = 0;
	int tempoffset = 0;
	// a matrix (left one) is already in row major format
	...
	for(int r=0; r<rows; r++){
		...
		for(int col=0; col<columns; col++){
			...
			for(int com=0; com<common_dim; com++){
				//cumu_ele += left_matrix_row_major[ta + com] * right_matrix_col_major[tb + com];
...
void matrixMultEigen(const vector<vector<myType>> &a, const vector<vector<myType>> &b, vector<vector<myType>> &c, 
					size_t rows, size_t common_dim, size_t columns,
				 	size_t transpose_a, size_t transpose_b)
{
	assert(transpose_a == 0 && transpose_b == 0);
				 	...
				 	size_t transpose_a, size_t transpose_b)
{
	...
	int offset = 0;
	int tempoffset = 0;
	//a is already in row major format
	//for(int i=0; i<rows; i++){
		//offset = i*common_dim;
		//for(int j=0; j<common_dim; j++){
			//left_matrix_row_major[offset + j] = a[i][j]; //Row major filling.
...
*/
	...
	uint64_t totalDataSent = commObject.totalDataSent;
	uint64_t totalDataReceived = commObject.totalDataReceived;
...
void zero_4D_vector(vector< vector< vector< vector<myType> > > >& vec, 
...
void subtract_2D_vectors(const vector< vector<myType> >& inp_l, 
							  const vector< vector<myType> >& inp_r, 
...
void add_2D_vectors(const vector< vector<myType> >& inp_l, 
						const vector< vector<myType> >& inp_r, 
						vector< vector<myType> >& out, 
						int d1, int d2)
{
...
void zero_2D_vector(vector< vector<myType> >& vec, 
					int d1, int d2)
{
	for(int i=0; i<d1; i++){
		for(int j=0; j<d2; j++){
...
void add_4D_vectors(vector< vector< vector< vector<myType> > > >& inp_l, 
					vector< vector< vector< vector<myType> > > >& inp_r, 
					vector< vector< vector< vector<myType> > > >& out, 
					int d1, int d2, int d3, int d4)
{
	for(int i=0; i<d1; i++){
		for(int j=0; j<d2; j++){
...
void subtract_4D_vectors(vector< vector< vector< vector<myType> > > >& inp_l, 
						 vector< vector< vector< vector<myType> > > >& inp_r, 
						 vector< vector< vector< vector<myType> > > >& out, 
						 int d1, int d2, int d3, int d4)
{
...
void flatten_4D_vector(vector< vector< vector< vector<myType> > > >& input, 
					   vector<myType>& output, 
					   int d1, int d2, int d3, int d4)
{
	for(int i=0;i<d1;i++){
		for(int j=0;j<d2;j++){
...
void deflatten_4D_vector(vector<myType>& input, 
						 vector< vector< vector< vector<myType> > > >& output, 
						 int d1, int d2, int d3, int d4)
{
	for(int i=0;i<d1;i++){
		for(int j=0;j<d2;j++){
...
void flatten_2D_vector(vector< vector<myType> >& input, 
					   vector<myType>& output, 
					   int d1, int d2)
{
	for(int i=0;i<d1;i++){
...
void deflatten_2D_vector(vector<myType>& input, 
						 vector< vector<myType> >& output, 
						 int d1, int d2)
{
	for(int i=0;i<d1;i++){
		for(int j=0;j<d2;j++){
...
void send_4D_vector(vector< vector< vector< vector<myType> > > >& input, 
					int d1, int d2, int d3, int d4)
{
	vector<myType> flat_input(d1*d2*d3*d4, 0);

	//Flatten and send.
	flatten_4D_vector(input, flat_input, d1, d2, d3, d4);

	sendVector<myType>(ref(flat_input), adversary(partyNum), d1*d2*d3*d4);

}

void receive_4D_vector(vector< vector< vector< vector<myType> > > >& recv, 
						int d1, int d2, int d3, int d4)
{
	vector<myType> flat_recv(d1*d2*d3*d4, 0);

	//Receive and deflatten.
	receiveVector<myType>(ref(flat_recv), adversary(partyNum), d1*d2*d3*d4);
...
void send_2_4D_vector(vector< vector< vector< vector<myType> > > >& input1,
					vector< vector< vector< vector<myType> > > >& input2,
					int d11, int d12, int d13, int d14,
					int d21, int d22, int d23, int d24)
{
	int32_t len1 = d11*d12*d13*d14;
	int32_t len2 = d21*d22*d23*d24;
	vector<myType> sendArr(len1 + len2, 0);
	for(int i=0;i<d11;i++){
		for(int j=0;j<d12;j++){
			for(int k=0;k<d13;k++){
				for(int l=0;l<d14;l++){
	...
	sendVector<myType>(ref(sendArr), adversary(partyNum), len1+len2);
}

void receive_2_4D_vector(vector< vector< vector< vector<myType> > > >& input1,
						vector< vector< vector< vector<myType> > > >& input2,
						int d11, int d12, int d13, int d14,
						int d21, int d22, int d23, int d24)
{
	int32_t len1 = d11*d12*d13*d14;
	int32_t len2 = d21*d22*d23*d24;
	vector<myType> rcvArr(len1 + len2, 0);
	receiveVector<myType>(ref(rcvArr), adversary(partyNum), len1+len2);
	for(int i=0;i<d11;i++){
		for(int j=0;j<d12;j++){
			for(int k=0;k<d13;k++){
				for(int l=0;l<d14;l++){
...
void send_2D_vector(vector< vector<myType> >& input, 
					int d1, int d2)
{
	vector<myType> flat_input(d1*d2, 0);

	//Flatten and send.
	flatten_2D_vector(input, flat_input, d1, d2);

	sendVector<myType>(ref(flat_input), PARTY_B, d1*d2);

}

void receive_2D_vector(vector< vector<myType> >& recv, 
					  int d1, int d2)
{
	vector<myType> flat_recv(d1*d2, 0);

	//Receive and deflatten.
	receiveVector<myType>(ref(flat_recv), PARTY_C, d1*d2);

	deflatten_2D_vector(flat_recv, recv, d1, d2);

}

void populate_4D_vector(vector< vector< vector< vector<myType> > > >& vec, 
						int d1, int d2, int d3, int d4, 
						string type)
{
	AESObject* aesObject;
...
void populate_2D_vector(vector< vector<myType> >& vec, 
						int d1, int d2, 
						string type)
{
	AESObject* aesObject;
...
/*
* NOTE : This function can easily be used for multi-dimensional arrays, since C++ is row-major format.
* 		 Link : https://dzone.com/articles/memory-layout-of-multi-dimensional-arrays-1
*/
void populate_AES_Arr(uint64_t* arr, uint64_t size, string type)
{
	AESObject* aesObject;
	if(type == "a1") aesObject = aes_conv_opti_a_1;
	else if(type == "a2") aesObject = aes_conv_opti_a_2;
...
void add_2_Arr(uint64_t* arr1, uint64_t* arr2, uint64_t* arr, uint64_t size)
{
	for(uint64_t i=0; i<size; i++){
		arr[i] = arr1[i] + arr2[i];
	}
...
void subtract_2_Arr(uint64_t* arr1, uint64_t* arr2, uint64_t* arr, uint64_t size)
{
	for(uint64_t i=0; i<size; i++){
		arr[i] = arr1[i] - arr2[i];
	}
